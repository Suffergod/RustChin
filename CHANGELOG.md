...
- **Project Homepage (`homepage_url`)**: Declared `https://rust-chin.ir/` in the manifest, so Chrome's extension details page and the Web Store listing both link to a real site instead of offering nowhere to go. Until this release the extension had no homepage at all.
- **First-Install Setup Guide**: `chrome.runtime.onInstalled` now opens `https://rust-chin.ir/welcome` on an install. The icon lands in Chrome's extensions menu rather than on the toolbar, so a new user has no visible sign the extension is present and no reason to think it is doing anything; a three-step Persian guide with the toolbar labels in both Persian and English is what turns an install into a working install. The tab opens on `details.reason === "install"` only, because `onInstalled` also fires on every update and on Chrome's own updates, and re-opening a tab under someone who has used the extension for months is an ambush, not onboarding.
...
- **Updated Store Promo Assets and Graphic Suite**: Replaced all store presentation assets in `promo_assets/` with the new claymorphic visual identity. Features high-fidelity 1280x800 slides for hero onboarding, Persian typography customization, DeepSeek code and table layout, and Claude reading ergonomics, paired with standardized 440x280 and 1400x560 promo tiles.
- **Store Summary and Feature Rebalance**: Rebalanced extension summaries (`appDesc`) across English and Persian to emphasize standard table layouts, ordered list numbering, and 5 Persian variable fonts alongside AI chat support, adhering strictly to Chrome's 132-character ceiling.
...
- **Settings Studio Reading Its Version From Static Markup**: The version pill in `options/options.html` was hardcoded and nothing ever rewrote it, so it was correct only because the manifest version and the hardcoded string happened to agree. It now reads `chrome.runtime.getManifest().version`, with the markup value kept as a fallback for a plain `file://` preview. The popup already read the manifest; the two surfaces now agree, and the next release cannot ship a dashboard that contradicts its own manifest.
